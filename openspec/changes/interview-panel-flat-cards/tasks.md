...
本 change 没有「只写代码就算完」。**实现者必须亲自打开本机已在使用的 DSH Desktop**，在 GUI 里走完：打开入口见到 `xerina-avatar.png` 圆形头像与选题壳 → 开始后见到 `Q1` 卡片流 → 点进详情看见题干与开卷要点 → 返回卡片流 → 宿主作答后本题对照/五维在详情可回看 → 等待或出现下一问时旧卡仍在且可见生成中 → 新卡成为当前并可切回上一张。否则整份 tasks 视为未完成，4.3 不得勾选，不得归档。
...
- [x] 2.3 实现生成中槽：`refreshing` 或最新卡 `status === 'scored'` 时可见「正在生成本题」或「正在准备下一题」；`cards.length` 增加时跳到新卡（详情则进新卡详情）；仅评分同一卡不打断历史浏览。单测覆盖生成中出现/新卡到达后消失
...
- [ ] 4.3 **【验收门槛 · 必须打开 DSH Desktop】** 按第 0 节在 GUI 对照 `docs/product-design/` 主路径图走完入口（真源头像）、卡片流、详情、返回、生成中、切回已评卡。勾选前填写：
...
作答后详情是否可见对照与五维（非前端计算）：未核验（未在宿主输入框提交本题作答）
生成中占位是否出现且旧卡仍在：未核验（依赖上一问已评或刷新本题的进行中态）
新卡是否自动成为当前、上一题能否切回：未核验（本场仅 Q1）
滑动或按钮切卡是否可用：按钮可见；本场仅 Q1，上一题/下一题禁用；多卡滑动未手测
面板是否仍无聊天输入/气泡：是
考场是否 JSON / 催问气泡：否
```
