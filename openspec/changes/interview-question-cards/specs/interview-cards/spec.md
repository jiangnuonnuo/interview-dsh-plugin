...
- **THEN** 面板仍停在 `Q1` 卡，可见对照（覆盖/缺口）与八股五维分数，工作区 `study/interview-dsh/` 下该场目录已有 `cards/Q1.md` 且含作答与分数
...
系统 SHALL 把本场甲板写入当前工作区目录 `study/interview-dsh/<开考时间>-<主题>/`：`session.json` 为机器可读真源，`cards/<编号>.md` 为人读副本。题目一出现 MUST 建立该卡文件；作答评分后 MUST 更新同一文件。关上面板再打开「面试」时，系统 MUST 从磁盘恢复全部卡片与当前卡。开始前 MUST NOT 弹出文件夹选择。查不到工作区目录或 `fs` 写入失败时，面板 MUST 展示可见错误，MUST NOT 丢掉已有卡片，MUST NOT 空 catch。

#### Scenario: Files appear with the first question
- **WHEN** `Q1` 开卷要点已生成且工作区目录已知
- **THEN** 该场目录存在 `session.json` 与 `cards/Q1.md`，其中含编号、题干与开卷要点

#### Scenario: Scoring updates the same markdown file
- **WHEN** `Q1` 完成对照与五维
- **THEN** `cards/Q1.md` 含作答、对照与分数，系统 MUST NOT 另起一份把 `Q1` 覆盖成下一题
