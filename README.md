...
- **Jev 定档**：引入可选的 Jev 机制，用来快速判断作答落在哪一档，从而决定留在原卡、开追问还是换下一题。开启后不必等对照评语写完再开卡。默认关闭；未填密钥、连通失败或返回无法映射时，仍走原来的对照补全。
- **评语与开口不变**：对照、五维和面试官开口继续用当前对话已配置的模型。Jev 不写评语、开卷要点或考场气泡。
- **追问上限**：同一知识点已经追问三次后，下一张改为下一个正式问。
- **密钥**：只保存在工作区 `.dsh-interview/config/jev.json`，不写入各轮档案，也不出现在卡片正文里。
